...
class AuxDecoder(nn.Module):
    ...
    def forward(self, x):
        for i in range(len(self.de_convs)):
            x = self.de_convs[i](x)
        return x
...
class GTCRN(nn.Module):
    ...
    def forward(self, x):
        ...
        aux_output = None
        if self.training:
            aux_feat = self.aux_decoder(feat)
            aux_m = self.erb.bs(aux_feat)
            aux_spec = self.mask(aux_m, spec)
            aux_spec = aux_spec.permute(0,3,2,1)
            aux_spec = torch.complex(aux_spec[...,0], aux_spec[...,1])
            aux_output = torch.istft(aux_spec, **stft_kwargs)
            aux_output = torch.nn.functional.pad(aux_output, (0, n_samples-aux_output.shape[1]))
