...
class MultiheadSelfAttention(nn.Module):
    def __init__(self, embed_dim: int, num_heads: int, seq_len: int, dropout: float = 0.0, bias: bool = True):
        super().__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.head_dim = embed_dim // num_heads

        self.positional_encoding = nn.Parameter(torch.empty(1, seq_len, embed_dim))
    ...
    def forward(self, query: torch.Tensor, need_weights: bool = False):
        query = query + self.positional_encoding
...
class DPGRNN(nn.Module):
    ...
    def __init__(self, input_size, width, hidden_size, **kwargs):
        ...
        self.intra_attn = MultiheadSelfAttention(embed_dim=input_size, num_heads=4, seq_len=33, dropout=0.0)
