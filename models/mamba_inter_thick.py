...
class DPGRNN(nn.Module):
    ...
    def __init__(self, input_size, width, hidden_size, **kwargs):
        ...
        self.inter_mamba = PureMambaBlock(hidden_size)
    
    def forward(self, x):
        """x: (B, C, T, F)"""
        ## Intra RNN
        x = x.permute(0, 2, 3, 1)  # (B,T,F,C)
        intra_x = self.intra_ln(x)
        ...
        inter_x = self.inter_mamba(inter_x)  # (B*F,T,C)
        inter_out = inter_x.reshape(B, F, T, C) # (B,F,T,C)
        inter_out = inter_out.permute(0,2,1,3)   # (B,T,F,C)
