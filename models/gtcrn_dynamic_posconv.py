...
class ERB(nn.Module):
    ...
    @staticmethod
    def erb_index_to_abs_freq_coords(index, erb_subband_1, erb_subband_2, nfft=512, high_lim=8000, fs=16000):
        new_subband1 = int((index * erb_subband_1) // (erb_subband_1 + erb_subband_2))
        new_subband2 = index - new_subband1
        low_lim = erb_subband_1/nfft * fs
        erb_low = ERB.hz2erb(low_lim)
        erb_high = ERB.hz2erb(high_lim)
        erb_points = np.linspace(erb_low, erb_high, new_subband2)
        band2_hertz_points = ERB.erb2hz(erb_points)
        band1_hertz_points = np.linspace(0, low_lim, new_subband1, endpoint=False)
        all_hertz_points = np.concatenate([band1_hertz_points, band2_hertz_points])
        all_hertz_points = all_hertz_points / high_lim * 2 - 1
        return all_hertz_points
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.en_convs = nn.ModuleList([
            ConvBlock(3*3+1, CHANNELS, (1,5), stride=(1,2), padding=(0,2), use_deconv=False, is_last=False),
...
class GTCRN(nn.Module):
    ...
    def forward(self, x):
        ...
        f_coords = torch.from_numpy(ERB.erb_index_to_abs_freq_coords(129, 65, 64)).to(device).float()  # (129,)
        f_coords = f_coords.reshape(1,1,1,-1).expand(feat.shape[0], 1, feat.shape[2], -1)
        feat = torch.cat([feat, f_coords], dim=1)  # (B,10,T,129)
