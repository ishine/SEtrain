...
class TRA(nn.Module):
    ...
    def __init__(self, channels, kernels, kat_heads, fa_kernel_size=(3,1), 
                 fa_expansion=5, fa_left_padding=2, fa_f_padding=0):
        super().__init__()
        ...
        self.left_padding = fa_left_padding
        self.fa_conv1 = nn.Conv2d(1, 1 * fa_expansion, fa_kernel_size, padding=(0, fa_f_padding))
        self.fa_conv2 = nn.Conv2d(1 * fa_expansion, 1, fa_kernel_size, padding=(0, fa_f_padding))
        self.fa_act1 = nn.PReLU()
        self.fa_act2 = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        zf = torch.mean(x.pow(2), dim=1, keepdim=True)  # (B, 1, T, F)
        zf = F.pad(zf, [0,0,self.left_padding,0])
        fattn = self.fa_conv1(zf)
        fattn = self.fa_act1(fattn)
        fattn = F.pad(fattn, [0,0,self.left_padding,0])
        fattn = self.fa_conv2(fattn)
        fattn = self.fa_act2(fattn)
        fattn = fattn.reshape(x.shape[0], x.shape[2], x.shape[3])  # (B,T,F)
        tfattn = at1.unsqueeze(-1) * fattn.unsqueeze(1)  # (B,C,T,F)
        return tfattn, kat
