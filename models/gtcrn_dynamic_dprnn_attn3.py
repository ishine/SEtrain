...
class TFE(nn.Module):
    """Sub-Temporal Feature Extraction"""
    def __init__(self, kernel_size=3, stride=1):
        super().__init__()
        self.kernel_size = kernel_size
        self.unfold = nn.Unfold(kernel_size=(kernel_size, 1), stride=(stride, 1), padding=((kernel_size-1)//2, 0))
...
class DPAttn(nn.Module):
    ...
    MAC_total = B·T·F · [ 8·C^2 + 4·H^2 + 2·C·(F + W) ]
    """
...
if __name__ == "__main__":
    ...
    #                                         print_per_layer_stat=True, verbose=False, backend='pytorch', 
    #                                         custom_modules_hooks={DPAttn: DPAttn.custom_module_flops_counter_hook}
    #                                         )
    # params = 0
