...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.en_convs = nn.ModuleList([
            ConvBlock(3, 8, (1,5), stride=(1,2), padding=(0,2), use_deconv=False, is_last=False),
            ConvBlock(8, 8, (1,5), stride=(1,2), padding=(0,2), groups=2, use_deconv=False, is_last=False),
            # ConvBlock(8, 8, (5,1), stride=(2,1), padding=(2,0), groups=2, use_deconv=False, is_last=False, time_padding=True),
            # ConvBlock(8, 8, (5,1), stride=(2,1), padding=(2,0), groups=2, use_deconv=False, is_last=False, time_padding=True),
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.de_convs = nn.ModuleList([
            GTConvBlock(8, 8, (3,3), stride=(1,1), padding=(5*2,1), dilation=(5,1), use_deconv=True),
            GTConvBlock(8, 8, (3,3), stride=(1,1), padding=(2*2,1), dilation=(2,1), use_deconv=True),
            GTConvBlock(8, 8, (3,3), stride=(1,1), padding=(1*2,1), dilation=(1,1), use_deconv=True),
            # ConvBlock(8, 8, (5,1), stride=(2,1), padding=(2,0), groups=2, use_deconv=True, is_last=False, time_padding=True),
            # ConvBlock(8, 8, (5,1), stride=(2,1), padding=(2,0), groups=2, use_deconv=True, is_last=False, time_padding=True),
            ConvBlock(8, 8, (1,5), stride=(1,2), padding=(0,2), groups=2, use_deconv=True, is_last=False),
            ConvBlock(8, 1, (1,5), stride=(1,2), padding=(0,2), use_deconv=True, is_last=True)
        ])
...
class GTCRN(nn.Module):
    def __init__(
        ...
        self.dpgrnn1 = DPGRNN(8, 33, 8)
        self.dpgrnn2 = DPGRNN(8, 33, 8)
    ...
    def forward(self, x, return_features=False):
        """
        x: (B, L)
        return_features: 是否返回隐藏层特征，用于知识蒸馏
        """
        device = x.device
        n_samples = x.shape[1]
        ...
        feat_dpgrnn1 = self.dpgrnn1(feat) # (B,8,T,33)
        feat_dpgrnn2 = self.dpgrnn2(feat_dpgrnn1) # (B,8,T,33)

        m_feat, de_outs = self.decoder(feat_dpgrnn2, en_outs)
        ...
        if return_features:
            # 返回所有关键层的特征输出，用于知识蒸馏
            features = {
                'encoder_outputs': en_outs,  # 编码器各层输出
                'dpgrnn1_output': feat_dpgrnn1,  # 第一个DPGRNN输出
                'dpgrnn2_output': feat_dpgrnn2,  # 第二个DPGRNN输出
                'decoder_outputs': de_outs,  # 解码器各层输出
                'decoder_input': feat_dpgrnn2,  # 解码器输入
                'mask_feature': m_feat,  # 掩码特征
                'final_mask': m  # 最终掩码
            }
...
if __name__ == "__main__":
    ...
    y1 = model(x1)[0]
    y2 = model(x2)[0]
    features1 = model.get_features(x1)
    features2 = model.get_features(x2)
