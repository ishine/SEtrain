...
class TRA(nn.Module):
    ...
    def __init__(self, channels, kernels):
        ...
        self.att_fc2 = nn.Linear(channels * 2, kernels)
    ...
    def forward(self, x):
        ...
        At = at1[..., None]  # (B,C,T,1)
        kat = self.att_fc2(at).transpose(1,2) # (B,kernels,T)
...
class GTConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, kernel_size, stride, padding, dilation, kernel_choices=8, use_deconv=False):
        ...
        self.tra = TRA(in_channels//2, kernel_choices)
    ...
    def forward(self, x):
        ...
        mask, kat = self.tra(x1)
        x1 = self.sfe(x1)
        h1 = self.point_act(self.point_bn1(self.point_conv1(x1, kat)))
        # h1 = nn.functional.pad(h1, [0, 0, self.pad_size, 0])
        h1 = self.depth_act(self.depth_bn(self.depth_conv(h1, kat)))
        h1 = self.point_bn2(self.point_conv2(h1, kat))
