...
class LearnableFB(nn.Module):
    def __init__(self, n_freqs, subbands, n_fft=512, fs=16000):
        ...
        if subbands < n_freqs:
            filters = self.erb_filter_banks(subbands, n_fft, fs)
            self.enc.weight.data = filters
            self.dec.weight.data = filters.T
    ...
    def erb_filter_banks(self, n_subbands, nfft=512, fs=16000):
        low_lim = 0
        high_lim = fs / 2
        erb_low = self.hz2erb(low_lim)
        erb_high = self.hz2erb(high_lim)
        erb_points = np.linspace(erb_low, erb_high, n_subbands + 2)
        bins = np.round(self.erb2hz(erb_points)/fs*nfft).astype(np.int32)
        erb_filters = np.zeros([n_subbands, nfft // 2 + 1], dtype=np.float32)

        for i in range(n_subbands):
            erb_filters[i, bins[i]:bins[i+1]] = (np.arange(bins[i], bins[i+1]) - bins[i] + 1e-12)\
                                                    / (bins[i+1] - bins[i] + 1e-12)
            erb_filters[i, bins[i+1]:bins[i+2]] = (bins[i+2] - np.arange(bins[i+1], bins[i + 2])  + 1e-12) \
...
class GTCRN(nn.Module):
    def __init__(
        ...
        self.fb = LearnableFB(n_fft // 2 + 1, subbands, n_fft=n_fft)
        self.sfe = SFE(3, 1)
    ...
    def forward(self, x):
        ...
        feat = self.fb.bm(feat)  # (B,3,T,subbands)
        feat = self.sfe(feat)     # (B,9,T,subbands)
