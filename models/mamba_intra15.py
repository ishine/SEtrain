...
class BidirectionalMambaBlock(nn.Module):
    def __init__(self, d_model):
        ...
        self.mamba_fwd = BiMamba(
            d_model=d_model,
            d_state=16, 
            d_conv=4,   
            expand=1.5
        )
        self.mamba_bwd = BiMamba(
            d_model=d_model,
            d_state=16, 
            d_conv=4,   
            expand=1.5
...
class DPGRNN(nn.Module):
    ...
    def __init__(self, input_size, width, hidden_size, **kwargs):
        ...
        self.intra_mamba = BidirectionalMambaBlock(hidden_size)

        self.inter_ln = nn.LayerNorm(hidden_size, eps=1e-8)
        self.inter_rnn = GRNN(input_size=hidden_size, hidden_size=hidden_size, bidirectional=False)
    ...
    def forward(self, x):
        ...
        intra_out = self.intra_mamba(intra_x)  # (B*T,F,C)
        intra_out = intra_out.reshape(B, T, F, C) # (B,T,F,C)

        ## Inter RNN
        x = intra_out.permute(0,2,1,3)  # (B,F,T,C)
        B, F, T, C = x.shape
        inter_x = self.inter_ln(x)
        inter_x = inter_x.reshape(B * F, T, C)
        inter_x = self.inter_rnn(inter_x)[0]
        inter_x = self.inter_fc(inter_x)
        
        inter_x = inter_x.reshape(B, F, T, C) # (B,F,T,C)
        inter_out = torch.add(x, inter_x)
        
        dual_out = inter_out.permute(0,3,2,1)  # (B,C,T,F)
