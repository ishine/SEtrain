...
# from models.mamba_hybrid_24 import GTCRN as Model
...
def run(rank, config, args):
    ...
    Model = import_model(config['model']['path'], config['model']['classname'])
...
class Trainer:
    def __init__(self, config, model, optimizer, scheduler, loss_func,
        ...
        if not self.resume:
            base_exp_path = self.trainer_config['exp_path'] + '_' + datetime.now().strftime("%Y-%m-%d-%Hh%Mm")
            self.exp_path = base_exp_path
            counter = 2
            while os.path.exists(self.exp_path):
                self.exp_path = f"{base_exp_path}-{counter}"
                counter += 1
        ...
        if self.rank == 0:
            ...
            # Copy models directory if it exists relative to the script
            if (Path(__file__).parent / 'models').exists():
                 shutil.copytree(Path(__file__).parent / 'models', Path(self.code_path) / 'models', dirs_exist_ok=True)
            self.writer = SummaryWriter(self.log_path)
            
            # --- Auto-Workflow: Save Metadata ---
            # Using standard dict for json serialization (removing objects that are not serializable)
            args_dict = vars(self.args_for_metadata).copy()
            ...
            metadata = {
                ...
                "model_classname": config['model']['classname'],
                "config_path": args_dict.get('config', 'unknown'),
                "exp_id": os.path.basename(self.exp_path),
                "exp_path": str(self.exp_path),
                "status": "training_started",
                "args": args_dict,
                "network_config": OmegaConf.to_container(config['network_config'], resolve=True)
            }
            with open(os.path.join(self.exp_path, 'metadata.json'), 'w') as f:
                json.dump(metadata, f, indent=4)
            # ------------------------------------
            
            # Print exp_path to stdout so manager can catch it easily
            print(f"EXP_PATH_GENERATED:{self.exp_path}")
    ...
    def _save_checkpoint(self, epoch, score):
        ...
        if score > self.best_score:
            ...
            # Update metadata with best score if possible
            if self.rank == 0:
                try:
                    with open(os.path.join(self.exp_path, 'metadata.json'), 'r') as f:
                        metadata = json.load(f)
                    metadata['best_score'] = float(score)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-C', '--config', default='configs/cfg_train_auto.yaml')
    ...
    # Merge command line args into config if using omegaconf dot notation (not done here yet, relying on config file or user passing full config via CLI merge if they want?)
    # The user asked to pass overrides via CLI. OmegaConf supports this.
    # But argparse here just takes -C and -D.
    # If we want to support overriding like `model.path=...` we need to parse extra args.
    # But for now, we stick to the plan: use OmegaConf.merge if needed, but simple way is config file.
    # We will let manager.py handle creating/modifying config OR we can use OmegaConf.from_cli()
    
    # Let's add support for overriding via CLI remaining args:
    # args, unknown = parser.parse_known_args()
    # But standard train.py doesn't have it. We should assume the config file passed is correct.
